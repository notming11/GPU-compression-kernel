    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            if hasattr(module, "lora_left"):
                module.lora_left.requires_grad = True
            if hasattr(module, "lora_right"):
                module.lora_right.requires_grad = True
...
def fine_tune(
    ...
    params_optim = [
        {
            'params' : [p for n, p in model.named_parameters() if "lora_left" in n],
            "lr" : learning_rate * lora_b_lr_coeff
        },
        {
            'params' : [p for n, p in model.named_parameters() if "lora_right" in n],
            "lr" : learning_rate
        },
    ]

    if optimizer_name == "adamw_torch":
        optimizer = optim.AdamW(params_optim, weight_decay=weight_decay)
    else:
        raise NotImplementedError(f"Optimizer {optimizer} not implemented.")
    lr_scheduler = get_cosine_schedule_with_warmup(
        optimizer=optimizer,
        num_warmup_steps=warmup_steps,
        num_training_steps=max_train_samples // global_batch_size,
    )
    ...
    if dataset_name == "c4":
        ...
        if os.path.exists(f"{cache_dir}/slimpajama.pt"):
            raw_datasets = load_from_disk(f"{cache_dir}/slimpajama.pt")
        else:
            raw_datasets = load_dataset("DKYoon/SlimPajama-6B", cache_dir=cache_dir)
            raw_datasets.save_to_disk(f"{cache_dir}/slimpajama.pt")
    else:
    ...
    if training_args.do_train and max_train_samples is not None:
        raw_datasets["train"] = raw_datasets["train"].select(range(min(len(raw_datasets["train"]), int(max_train_samples*6))))
    ...
    cache_file_names = {
        "train" : f"{cache_dir}/{dataset_name}.pt/train/{model.config.name_or_path.split('/')[-1]}_train_tokenize_{block_size}.arrow",
        "validation" : f"{cache_dir}/{dataset_name}.pt/validation/{model.config.name_or_path.split('/')[-1]}_val_tokenize_{block_size}.arrow",
        "test" : f"{cache_dir}/{dataset_name}.pt/test/{model.config.name_or_path.split('/')[-1]}_test_tokenize_{block_size}.arrow",
    }
    ...
    cache_file_names = {
        "train" : f"{cache_dir}/{dataset_name}.pt/train/{model.config.name_or_path.split('/')[-1]}_train_group_{block_size}.arrow",
        "validation" : f"{cache_dir}/{dataset_name}.pt/validation/{model.config.name_or_path.split('/')[-1]}_val_group_{block_size}.arrow",
        "test" : f"{cache_dir}/{dataset_name}.pt/test/{model.config.name_or_path.split('/')[-1]}_test_group_{block_size}.arrow",
    }
