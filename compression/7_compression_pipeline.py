...
def sparse_persistent_matmul_pipelined_kernel(
    ...
    a_warp_bases: gl.constexpr = (
        [[16, 0], [32, 0]] if num_warps == 4 else ([[16, 0], [32, 0], [64, 0]] if num_warps == 8 else [[16, 0], [32, 0], [64, 0], [128, 0]])
    )
    ...
    a_pruned_reg_layout: gl.constexpr = gl.DistributedLinearLayout(
        ...
        block_bases=[],
        shape=[64, 64] if num_warps == 4 else ([128, 64]if num_warps == 8 else [256, 64]),
    )
    ...
    meta_idx_layout: gl.constexpr = gl.DistributedLinearLayout(
        ...
        warp_bases=a_warp_bases,
        block_bases=[],
        shape=[64, 8] if num_warps == 4 else ([128, 8]if num_warps == 8 else [256, 8]),
    )
    meta_col = (
        gl.arange(0, BLOCK_K // 16, gl.SliceLayout(0, a_pruned_reg_layout))[None, :] * 4
    )
    meta_row = gl.arange(0, BLOCK_M, gl.SliceLayout(1, a_pruned_reg_layout))[:, None] * 0
