...
# PTX for intra-quad 4x4 register transpose (inlined as module-level constant
# to avoid @gluon.jit tracer converting the string literal to a tensor)
LAYOUT_TRANSPOSE_PTX = """
{
    ...
    .reg .b32 %quad_lane;
    and.b32 %quad_lane, %lane, 3;

    .reg .b32 %bit0, %bit1;
    and.b32 %bit0, %quad_lane, 1;
    and.b32 %bit1, %quad_lane, 2;

    .reg .pred %p1, %p2;
    setp.ne.u32 %p1, %bit0, 0;
    setp.ne.u32 %p2, %bit1, 0;
    ...
    shfl.sync.bfly.b32 %e1, %s1, 1, 0x1f, 0xffffffff;
    shfl.sync.bfly.b32 %e3, %s3, 1, 0x1f, 0xffffffff;
    ...
    .reg .b32 %s2, %s3_new, %e2, %e3_new;
    selp.b32 %s2, %r0, %r2, %p2;
    selp.b32 %s3_new, %r1, %r3, %p2;

    shfl.sync.bfly.b32 %e2, %s2, 2, 0x1f, 0xffffffff;
    shfl.sync.bfly.b32 %e3_new, %s3_new, 2, 0x1f, 0xffffffff;

    @%p2  mov.b32 %r0, %e2;
    @!%p2 mov.b32 %r2, %e2;
    @%p2  mov.b32 %r1, %e3_new;
    @!%p2 mov.b32 %r3, %e3_new;
...
def small_mma_kernel(
    ...
    # =====================================================================
    # 1. SPLIT: Decompose the compressed tensor into 4 sub-tensors
    a_grouped_4 = a_compressed.reshape(BLOCK_M, BLOCK_K // 8, 2, 2)

    a_split_even, a_split_odd = a_grouped_4.split()
    x0, x2 = a_split_even.split()
    x1, x3 = a_split_odd.split()

    # Bitcast fp16 pairs to int32 for the PTX '=r' register constraints
    x0 = x0.to(gl.int32)
    x1 = x1.to(gl.int32)
    x2 = x2.to(gl.int32)
    x3 = x3.to(gl.int32)

    # 2. EXECUTE PTX: Perform the intra-quad 4x4 register transpose (inlined)
    # TRANSPOSE_PTX: gl.constexpr = LAYOUT_TRANSPOSE_PTX
    (y0, y1, y2, y3) = gl.inline_asm_elementwise(
        TRANSPOSE_PTX,
        "=r,=r,=r,=r,r,r,r,r",
        [x0, x1, x2, x3],
        dtype=(gl.int32, gl.int32, gl.int32, gl.int32),
        is_pure=True,
        pack=1,
    )

    # 3. Bitcast back to fp16 pairs and reassemble
    y0 = y0.to(gl.float16)
    y1 = y1.to(gl.float16)
    y2 = y2.to(gl.float16)
    y3 = y3.to(gl.float16)

    y_even = gl.join(y0, y2)
    y_odd = gl.join(y1, y3)
    y_grouped = gl.join(y_even, y_odd)
    y_reshaped = y_grouped.reshape(BLOCK_M, BLOCK_K // 2)

    # 4. ZERO-COST CAST
    a_compressed = gl.convert_layout(y_reshaped, a_compressed_layout, assert_trivial=False)
    # =====================================================================
...
if __name__ == "__main__":
    ...
    os.environ["MLIR_DUMP_PATH"] = "./MLIR_DUMP/3I"
