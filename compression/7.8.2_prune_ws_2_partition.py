...
class SparseWGMMA:
    ...
    @gluon.jit
    def generate_compressed_and_meta_raw(self, a_dense, BLOCK_M : gl.constexpr, BLOCK_K: gl.constexpr):
...
class SparsePartitionArgs:
    ...
    def __init__(self, a_desc, b_desc, c_desc, 
                 a_bufs, b_bufs, a_comp_bufs, e_bufs,
...
def sparse_matmul_load_and_compress_partition(p, SchedulerImpl: gl.constexpr):
    ...
    compress_helper = SparseWGMMA.initialize(p.a_desc.dtype, BLOCK_M, BLOCK_N, compress_warps)
    ...
    for idx in range(scheduler.get_num_tiles()):
        ...
        for k_issue in range(0, (num_buffers - 1) * BLOCK_K, BLOCK_K):
            if k_issue < K:
                ...
                tma.async_copy_global_to_shared(p.a_desc, [off_m, k_issue], a_bar, p.a_bufs.index(issue_state.index))
        ...
        for k_process in range(0, K, BLOCK_K):
            ...
            a_dense = p.a_bufs.index(process_state.index).load(a_reg_layout)
            ...
            if k_future < K:
                ...
                mbarrier.expect(a_bar, p.a_desc.block_type.nbytes)
                tma.async_copy_global_to_shared(p.a_desc, [off_m, k_future], a_bar, p.a_bufs.index(issue_state.index))
...
def sparse_matmul_warp_specialized_kernel(
    a_desc, a_comp_desc, e_desc, b_desc, c_desc, SchedulerImpl: gl.constexpr,
...
def run_sparse_ws_matmul(A, B, tune=True, manual_config=None):
    ...
    a_comp_desc = TensorDescriptor.from_tensor(A, dummy_block, dummy_layout_f16)
    e_desc = TensorDescriptor.from_tensor(A.to(torch.int16), dummy_block, dummy_layout_i16)
    ...
    if tune:
        ...
        sparse_ws_kernel_autotune[grid](a_desc, a_comp_desc, e_desc, b_desc, c_desc, GroupedPersistentTileScheduler(8), M, N, K)
    else:
        hook_kwargs = {
            "BLOCK_SIZE_M": manual_config["BM"], "BLOCK_SIZE_N": manual_config["BN"],
            "BLOCK_SIZE_K": manual_config["BK"], "SUBTILE_FACTOR": manual_config["SF"],
            "a_desc": a_desc, "a_comp_desc": a_comp_desc, "e_desc": e_desc,
        ...
        sparse_matmul_warp_specialized_kernel[grid](
            a_desc, a_comp_desc, e_desc, b_desc, c_desc, GroupedPersistentTileScheduler(8),
            M, N, K,
...
if __name__ == "__main__":
    ...
    os.environ["MLIR_DUMP_PATH"] = "./MLIR_DUMP/7.8.2"
