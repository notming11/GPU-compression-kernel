#!/bin/bash
#SBATCH --job-name=7.5_profiling        # Job name
#SBATCH --output=7.5_16_profiling_%j.out    # Output file (%j = job ID)
...
apptainer exec --nv $SCRATCH/sparse.sif ncu --set full -f -k "sparse_persistent_matmul_pipelined_kernel" -o "Profiling/max_shape/7.5" python "7.5_compression_pipeline_no_ldmatrix.py"