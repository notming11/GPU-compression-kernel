...
# trim search space to save time benchmarking
ws_kernel_autotune_trimmed = triton.autotune(
    configs=matmul_get_trimmed_configs(
...
ws_kernel_autotune_768 = triton.autotune(
    configs=matmul_get_768_configs(
        pre_hook=matmul_tma_set_block_size_hook
    ),
    key=["M", "N", "K"],
    do_bench = lambda kernel_call, quantiles: triton.testing.do_bench_cudagraph(
...
ws_kernel_autotune = triton.autotune(
    configs=matmul_get_configs(pre_hook=matmul_tma_set_block_size_hook, tune=True),
    key=["M", "N", "K"],
    do_bench = lambda kernel_call, quantiles: triton.testing.do_bench_cudagraph(
        kernel_call, rep=100, quantiles=quantiles),
)(matmul_warp_specialized_kernel)

ws_kernel_single = triton.autotune(
    configs=matmul_get_configs(pre_hook=matmul_tma_set_block_size_hook, tune=False),
    key=["M", "N", "K"],
)(matmul_warp_specialized_kernel)


def run_ws_matmul(A, B, tune=True, manual_config=None):
    ...
    if tune:
        ...
        if M <= 768:
            ws_kernel_autotune_768[grid](a_desc, b_desc, c_desc, GroupedPersistentTileScheduler(8), M, N, K)
        else: 
            ws_kernel_autotune_trimmed[grid](a_desc, b_desc, c_desc, GroupedPersistentTileScheduler(8), M, N, K)
        ...
        matmul_tma_set_block_size_hook(hook_kwargs)
        ...
        matmul_warp_specialized_kernel[grid](
            a_desc, b_desc, c_desc, GroupedPersistentTileScheduler(8),
            M, N, K,
            BLOCK_SIZE_M=manual_config["BM"], 
            BLOCK_SIZE_N=manual_config["BN"], 
            BLOCK_SIZE_K=manual_config["BK"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Dense Warp-Specialized Matmul")
    ...
    for M, N, K in sizes:
        ...
        D = run_ws_matmul(A, B, tune=args.tune, manual_config=manual_config)
