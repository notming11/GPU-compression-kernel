...
@gluon.jit
def _s_tile_split(x):
    """Split [M, N] -> two [M, N//2] (interleaved columns)."""
...
@gluon.jit
def pipelined_softmax(
    S_tile, m_old, l_old, sm_scale_log2, dtype: gl.constexpr, p_layout: gl.constexpr, s_layout: gl.constexpr
):
    """
    SFU/ALU Pipelined Softmax:
    Overlaps MUFU exp2 on tile half 1 with CUDA ALU cast + sum reductions on tile half 0.
    """
    S_scaled = S_tile * sm_scale_log2

    # 1. Global row max computed on full tile to preserve WGMMA s_layout
    m_new = gl.maximum(m_old, gl.max(S_scaled, axis=1))
    rescale_factor = gl.exp2(m_old - m_new)

    # 2. Split normalized scores into sub-tiles along N
    S_norm = S_scaled - m_new[:, None]
    S_h0, S_h1 = _s_tile_split(S_norm)

    # 3. Interleaved SFU / ALU execution
    P_h0 = gl.exp2(S_h0)                      # Issue SFU on half 0
    P_h1 = gl.exp2(S_h1)                      # Issue SFU on half 1 (runs on SFU while ALUs execute below)

    l_h0 = gl.convert_layout(gl.sum(P_h0, axis=1), s_layout)  # ALU: overlaps SFU h1
    P_h0_cast = gl.cast(P_h0, dtype=dtype)                    # ALU: overlaps SFU h1

    l_h1 = gl.convert_layout(gl.sum(P_h1, axis=1), s_layout)  # ALU: post SFU h1
    P_h1_cast = gl.cast(P_h1, dtype=dtype)

    # 4. Accumulate l_new (all terms conform to s_layout)
    l_new = l_old * rescale_factor + l_h0 + l_h1

    # 5. Reconstruct full matrix & convert to WGMMA dot operand layout
    P_joined = _s_tile_join(P_h0_cast, P_h1_cast)
    P_cur_permuted = gl.convert_layout(P_joined, p_layout)

    return P_cur_permuted, m_new, l_new, rescale_factor
...
def fa3_consumer_wg0(p: PartitionArgs, SchedulerImpl: gl.constexpr, SEQ_LEN: gl.constexpr, NUM_HEADS: gl.constexpr, HEAD_DIM: gl.constexpr, p_layout: gl.constexpr, m_layout: gl.constexpr, s_layout: gl.constexpr):
    ...
    for tile_idx in range(scheduler.get_num_tiles()):
        ...
        P_cur_permuted, m_old, l_old, _prologue_rescale = pipelined_softmax(S_tile, m_old, l_old, sm_scale_log2, dtype, p_layout, s_layout)
        ...
        for step in range(1, num_steps - 1):
            ...
            P_cur_permuted, m_old, l_old, rescale_factor = pipelined_softmax(S_tile, m_old, l_old, sm_scale_log2, dtype, p_layout, s_layout)
