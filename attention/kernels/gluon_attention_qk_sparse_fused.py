...
class PartitionArgs:
    ...
    def __init__(
        ...
        q0_buf, q1_buf, q0_comp_buf, q1_comp_buf, eq0_buf, eq1_buf, k_bufs, v_bufs, o0_bufs, o1_bufs, 
...
def fa3_consumer_wg0(p: PartitionArgs, SchedulerImpl: gl.constexpr, SEQ_LEN: gl.constexpr, NUM_HEADS: gl.constexpr, HEAD_DIM: gl.constexpr, p_layout: gl.constexpr, m_layout: gl.constexpr, s_layout: gl.constexpr):
    ...
    for tile_idx in range(scheduler.get_num_tiles()):
        ...
        q0_comp, eq0_reg = dense_to_2_4_sparse(p.q0_buf, p.num_warps, SUB_BM, BLOCK_K)

        mbarrier.wait(p.kv_ready_bars.index(kv_state.index), kv_state.phase)
        mma_s = mma_s_base.issue_async_sparse_mma(q0_comp, eq0_reg, p.k_bufs.index(kv_state.index).permute((1, 0)))
        ...
        for step in range(1, num_steps - 1):
            ...
            mma_s = mma_s_base.issue_async_sparse_mma(q0_comp, eq0_reg, p.k_bufs.index(next_kv_state.index).permute((1, 0)))
...
def fa3_consumer_wg1(p: PartitionArgs, SchedulerImpl: gl.constexpr, SEQ_LEN: gl.constexpr, NUM_HEADS: gl.constexpr, HEAD_DIM: gl.constexpr, p_layout: gl.constexpr, m_layout: gl.constexpr, s_layout: gl.constexpr):
    ...
    for tile_idx in range(scheduler.get_num_tiles()):
        ...
        q1_comp, eq1_reg = dense_to_2_4_sparse(p.q1_buf, p.num_warps, SUB_BM, BLOCK_K)

        mbarrier.wait(p.kv_ready_bars.index(kv_state.index), kv_state.phase)
        mma_s = mma_s_base.issue_async_sparse_mma(q1_comp, eq1_reg, p.k_bufs.index(kv_state.index).permute((1, 0)))
        ...
        for step in range(1, num_steps - 1):
            ...
            mbarrier.wait(p.kv_ready_bars.index(next_kv_state.index), next_kv_state.phase)
            mma_s = mma_s_base.issue_async_sparse_mma(q1_comp, eq1_reg, p.k_bufs.index(next_kv_state.index).permute((1, 0)))
...
def fa3_warp_specialized_kernel(
    ...
    layout_q_comp: gl.constexpr = gl.NVMMASharedLayout.get_default_for(q0_comp_shape, dtype)
    layout_eq: gl.constexpr = gl.NVMMASharedLayout.get_default_for(eq0_shape, gl.int16)
    ...
    p = PartitionArgs(
        q0_desc, q1_desc, k_desc, v_desc, o0_desc, o1_desc, k_bufs, v_bufs, o0_bufs, o1_bufs,
        q_ready_bar, q_empty_bar, 
        kv_empty_bars, kv_ready_bars,
        o0_empty_bars, o0_ready_bars,
        o1_empty_bars, o1_ready_bars,
...
def run_fa3_sparse_q_kernel(Q_dense, K, V, tune=True, manual_config=None):
    ...
    # 1. Build TMA Descriptors directly from dense Q tensor
