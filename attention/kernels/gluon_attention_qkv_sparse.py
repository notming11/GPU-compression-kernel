...
os.environ["PYTORCH_KERNEL_CACHE_PATH"] = os.path.join(SCRATCH_WORKSPACE, f"torch_kernel_cache_{JOB_ID}")
...
def fa3_consumer_wg0(p: PartitionArgs, SchedulerImpl: gl.constexpr, SEQ_LEN: gl.constexpr, NUM_HEADS: gl.constexpr, HEAD_DIM: gl.constexpr, p_layout: gl.constexpr, m_layout: gl.constexpr, s_layout: gl.constexpr):
    ...
    for tile_idx in range(scheduler.get_num_tiles()):
        ...
        mma_o = WGMMA.initialize(dtype, SUB_BM, BLOCK_K, p.num_warps, sparse=True)
        ...
        for step in range(1, num_steps - 1):
            ...
            mma_o = mma_o.issue_async_sparse_mma(P_reg, P_meta_reg, p.v_bufs.index(kv_state.index))
            ...
            mma_o = WGMMA(o_acc, gl.to_tensor(True), mma_o.layout, SUB_BM, BLOCK_K, sparse=True)
...
def fa3_warp_specialized_kernel(
    ...
    p_layout: gl.constexpr = gl.DotOperandLayout(
        operand_index=0,
        parent=pick_wgmma_layout(dtype, SUB_BM, BLOCK_SIZE_K, num_warps, sparse=True),
        k_width=32 // dtype.primitive_bitwidth,
        meta=0,
    )
    
    m_layout: gl.constexpr = gl.SliceLayout(dim=1, parent=pick_wgmma_layout(dtype, SUB_BM, BLOCK_SIZE_K, num_warps, sparse=True))
    s_layout: gl.constexpr = gl.SliceLayout(dim=1, parent=pick_wgmma_layout(dtype, SUB_BM, BLOCK_SIZE_N, num_warps, sparse=True))
